...
async fn main() {
    ...
    match cli.command {
        Some(Command::Init) => {
            ...
            println!("Initialized FacetQL data directory at {}", config::data_dir().display());
        }
        Some(Command::Start { port, tls_identity, tls_identity_password }) => {
            run_server(port, tls_identity, tls_identity_password).await
        }
        None => run_server(8080, None, None).await,
        Some(Command::Backup { output_dir }) => run_backup(output_dir),
        Some(Command::Restore { input_dir }) => run_restore(input_dir),
        Some(Command::Import { source }) => run_import(source).await,
    }
}
...
fn run_backup(output_dir: PathBuf) {
    std::fs::create_dir_all(&output_dir).expect("failed to create backup output directory");
...
fn run_restore(input_dir: PathBuf) {
    ...
    if !existing.is_empty() {
        eprintln!(
            "refusing to restore: the target data directory ({}) already has data \
             ({}). Move it aside first if you really want to overwrite it.",
            config::data_dir().display(),
            existing.join(", ")
        );
        std::process::exit(1);
...
async fn run_import(source: ImportSource) {
    match source {
        ImportSource::Postgres { pg_url, table, kind, id_column, server, token } => {
            println!("Importing table '{table}' from Postgres into FacetQL ({server})...");
            match importer::import_postgres_table(&pg_url, &table, &kind, "", &id_column, &server, &token).await {
                Ok(summary) => {
                    println!("Imported {} row(s).", summary.imported);
                    if !summary.failed.is_empty() {
                        println!("{} row(s) failed:", summary.failed.len());
...
async fn run_server(port: u16, tls_identity: Option<PathBuf>, tls_identity_password: Option<String>) {
    ...
    match tls_identity {
        Some(identity_path) => {
            let password = tls_identity_password.unwrap_or_else(|| {
                eprintln!(
                    "warning: --tls-identity was set without --tls-identity-password — \
                     trying an empty password. This will fail for any real PKCS#12 file."
                );
                String::new()
            });

            let identity_bytes = std::fs::read(&identity_path).unwrap_or_else(|e| {
                panic!("failed to read TLS identity file {}: {e}", identity_path.display())
            });
            let identity = native_tls::Identity::from_pkcs12(&identity_bytes, &password)
                .unwrap_or_else(|e| panic!("failed to load TLS identity: {e}"));
            let acceptor: tokio_native_tls::TlsAcceptor =
                native_tls::TlsAcceptor::new(identity).unwrap().into();

            println!("FacetQL Server Running on port {port} (HTTPS)");
            tls_server::serve_tls(listener, app, acceptor).await;
        }
        None => {
            println!(
                "FacetQL Server Running on port {port} (HTTP — no TLS; either pass \
                 --tls-identity/--tls-identity-password for native HTTPS, or terminate \
                 TLS at a reverse proxy in front of this, before running with real traffic)"
            );
